...
export default function Dashboard() {
  ...
  const [selectedSymbol, setSelectedSymbol] = useState(() => {
    ...
    return (fromUrl && getInstrument(fromUrl)) ? fromUrl : 'RELIANCE'
  })

  // If the global search bar deep-links here with ?symbol=..., jump to it.
  useEffect(() => {
    const sym = searchParams.get('symbol')
    if (sym && getInstrument(sym)) {
        ...
        <div className="card">
          {selectedStock && selectedPrice && (
            <>
              ...
              {!isMarketOpen() && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid var(--red)', borderRadius: 6, padding: '8px 10px', marginBottom: 12, fontSize: 11, color: 'var(--red)', lineHeight: 1.5 }}>
                  Market closed · NSE/BSE hours: Mon–Fri, 9:15 AM – 3:30 PM IST
                </div>
              )}
